...
class TicketClaimSystem(commands.Cog):
    ...
    async def _send_layout(self, channel, *, title, body, colour=None):
        """Send a Components V2 message. Every reply from this cog goes
        through here so success, denial, warnings, and errors all look and
        behave the same way."""
    ...
    # ---------- global error handling for this cog ----------
    ...
    async def cog_command_error(self, ctx, error):
        ...
        # Anything else is unexpected — log it, then tell the user to close
        # the ticket rather than let the traceback go unanswered.
        print(f"[TicketClaimSystem] Unexpected error in {ctx.command}: {error!r}")
    ...
    async def on_thread_ready(self, thread, creator, category, initial_message):
        try:
            ...
            print(f"[TicketClaimSystem] on_thread_ready failed: {e!r}")
    ...
    async def claim(self, ctx, target: discord.Member = None):
        ...
        if thread is None:
            return await self._send_layout(
                ctx.channel, title="Not a Ticket", body="This isn't a ticket channel.", colour=RED_COLOUR
            )
        ...
        if state.get("claimed_by"):
            return await self._send_layout(
                ...
                body=f"This ticket is already claimed by <@{state['claimed_by']}>.",
                colour=RED_COLOUR,
            )
        ...
        if state.get("noclaim") and not is_bypass:
            return await self._send_layout(
                ...
                body="🚫 This ticket is restricted — only a claim-bypass role holder can claim it.",
                colour=RED_COLOUR,
            )
        ...
        if target is not None and target.id != ctx.author.id and not is_bypass:
            return await self._send_layout(
                ctx.channel,
                title="Permission Denied",
                body="Only a claim-bypass role holder can assign this ticket to someone else.",
                colour=RED_COLOUR,
            )
        ...
        if lock_warning:
            await self._send_layout(
                ctx.channel, title="Warning", body=f"⚠️ {lock_warning}", colour=YELLOW_COLOUR
            )
    ...
    async def unclaim(self, ctx, target: discord.Member = None):
        ...
        if not state.get("claimed_by"):
            return await self._send_layout(
                ctx.channel, title="Not Claimed", body="This ticket isn't claimed.", colour=RED_COLOUR
            )
        ...
        if state["claimed_by"] != ctx.author.id and not is_bypass:
            return await self._send_layout(
                ctx.channel,
                title="Permission Denied",
                body="Only the person who claimed this ticket (or a claim-bypass role holder) can unclaim it.",
                colour=RED_COLOUR,
            )
        ...
        if target is not None and target.id != state["claimed_by"]:
            return await self._send_layout(
                ctx.channel,
                title="Mismatch",
                body=f"This ticket isn't claimed by {target.mention}.",
                colour=RED_COLOUR,
            )
    ...
    async def subuser(self, ctx, member: discord.Member):
        ...
        if not state.get("claimed_by"):
            return await self._send_layout(
                ctx.channel, title="Not Claimed", body="This ticket isn't claimed yet.", colour=RED_COLOUR
            )
        ...
        if ctx.author.id != state["claimed_by"] and not is_bypass:
            return await self._send_layout(
                ctx.channel,
                title="Permission Denied",
                body="Only the person who claimed this ticket (or a claim-bypass role holder) can add subs.",
                colour=RED_COLOUR,
            )
        if member.id in state.get("subs", []):
            return await self._send_layout(
                ...
                body=f"{member.mention} is already subbed on this ticket.",
                colour=RED_COLOUR,
            )
    ...
    async def unsubuser(self, ctx, member: discord.Member):
        ...
        if member.id not in state.get("subs", []):
            return await self._send_layout(
                ctx.channel,
                title="Not Subbed",
                body=f"{member.mention} isn't subbed on this ticket.",
                colour=RED_COLOUR,
            )
        ...
        if ctx.author.id != state["claimed_by"] and not is_bypass:
            return await self._send_layout(
                ctx.channel,
                title="Permission Denied",
                body="Only the person who claimed this ticket (or a claim-bypass role holder) can remove subs.",
                colour=RED_COLOUR,
            )
    ...
    async def hold(self, ctx):
        ...
        if not state.get("claimed_by"):
            return await self._send_layout(
                ctx.channel,
                title="Not Claimed",
                body="This ticket needs to be claimed before it can go on hold.",
                colour=RED_COLOUR,
            )
        ...
        if ctx.author.id != state["claimed_by"] and not is_bypass:
            return await self._send_layout(
                ctx.channel,
                title="Permission Denied",
                body="Only the person who claimed this ticket (or a claim-bypass role holder) can toggle hold.",
                colour=RED_COLOUR,
            )
    ...
    async def noclaim(self, ctx):
        ...
        if state["noclaim"]:
            await self._send_layout(
                ctx.channel,
                title="Claim Restricted",
                body="🚫 This ticket can now only be claimed by users with a claim-bypass role.",
                colour=YELLOW_COLOUR,
            )
        else:
            await self._send_layout(
                ...
                body="This ticket can now be claimed by any staff member again.",
                colour=GREEN_COLOUR,
            )
    ...
    async def claimbypass(self, ctx, role: discord.Role):
        ...
        if role.id in config["bypass_roles"]:
            ...
            return await self._send_layout(
                ...
                body=f"{role.mention} removed from claim-bypass roles.",
                colour=RED_COLOUR,
            )
